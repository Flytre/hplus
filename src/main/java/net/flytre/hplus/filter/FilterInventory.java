...
public class FilterInventory implements Inventory{
    ...
    private void fromTag(CompoundTag tag) {
        this.inventory_height = tag.contains("height") ? tag.getInt("height") : 3;

        this.items = DefaultedList.ofSize(9 * inventory_height, ItemStack.EMPTY);
    ...
    public CompoundTag toTag() {
        ...
        tag.putInt("height", inventory_height);
    ...
    public void onClose(PlayerEntity player) {
        ...
        if (!player.getMainHandStack().hasTag()) {
            player.getMainHandStack().setTag(new CompoundTag());
        }

        player.getMainHandStack().getTag().put("filter", toTag());
