...
public class FilterUpgrade extends Item implements HopperUpgrade {
    ...
    public static FilterInventory getInventory(ItemStack stack) {
        ...
        if (!stack.getTag().contains("filter")) {
            stack.getTag().put("filter", new CompoundTag());
        }

        int state = stack.getTag().getInt("filterType");
        return new FilterInventory(stack.getTag().getCompound("filter"),state);
    }
